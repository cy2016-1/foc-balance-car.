...
//陀螺仪方向设置
//方向矩阵: 陀螺仪所在系统XYZ三个方向，每个方向对应陀螺仪的XYZ三个方向，正负区分同向反向
...
static void run_self_test(void)
{
	...
	if (result == 0x7) {
		...
		DEBUG_P("setting bias succesfully ......\r\n");
	}
}
...
{ 
	...
	DEBUG_P("mpu_set_sensor complete ......\r\n");
	...
	if(!mpu_init())
	{
		...
		if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL))
			DEBUG_P("mpu_configure_fifo complete ......\r\n");
		if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))
			DEBUG_P("mpu_set_sample_rate complete ......\r\n");
		if(!dmp_load_motion_driver_firmware())
			DEBUG_P("dmp_load_motion_driver_firmware complete ......\r\n");
		if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation)))
			DEBUG_P("dmp_set_orientation complete ......\r\n");
		if(!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
				DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
				DMP_FEATURE_GYRO_CAL))
			DEBUG_P("dmp_enable_feature complete ......\r\n");
		if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ))
			DEBUG_P("dmp_set_fifo_rate complete ......\r\n");
		
		run_self_test();
			
		if(!mpu_set_dmp_state(1))
		 DEBUG_P("mpu_set_dmp_state complete ......\r\n");
	}
}
/**************************************************************************
