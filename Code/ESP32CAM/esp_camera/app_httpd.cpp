...
#else
#include "esp_log.h"
static const char *TAG = "ESP32_Tracker";
#endif
...
static char* parse_get_str(char *buf, const char* key, char *def)
{
  static char _str[16];
  memset(_str, 0x0, sizeof(_str));
  if(httpd_query_key_value(buf, key, _str, sizeof(_str)) != ESP_OK){
      return def;
  }
  return _str;
}
...
static esp_err_t rocker_handler(httpd_req_t *req){
    char *buf = NULL;
    ...
    int x = parse_get_var(buf, "x", 0);
    int y = parse_get_var(buf, "y", 0);
    int sw = parse_get_var(buf, "sw", 0);
    free(buf);

    if(sw == 0){
      led_flag = !led_flag;
      led_duty = led_flag ? 127 : 0;
      enable_led(led_flag);
      delay(200);
    }
...
static esp_err_t command_handler(httpd_req_t *req){
    ...
    int type = parse_get_var(buf, "type", 0);
    char *cmd = parse_get_str(buf, "cmd", "");
    free(buf);

    String result = command_process(type, cmd);
...
void startCameraServer()
{
    ...
    httpd_uri_t rocker_uri = {
        .uri = "/rocker",
        .method = HTTP_GET,
        .handler = rocker_handler,
        .user_ctx = NULL};
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        //Other API
        httpd_register_uri_handler(camera_httpd, &rocker_uri);
